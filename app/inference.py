...
def load_audio(path: str, max_duration: int = 300) -> torch.Tensor:
    # Load audio using torchaudio for all formats (wav, mp3, flac, ogg, etc.)
...
def get_speaker_latent_and_mask(
    ...
    for i in range(0, audio.shape[1], audio_chunk_size):
        ...
        # Clean up chunk to avoid holding onto memory
        del audio_chunk, latent_chunk
    
    speaker_latent = torch.cat(latent_arr, dim=1)
    del latent_arr  # Clean up list after concatenation
...
def sample_pipeline(
    ...
    del text_input_ids, text_mask, speaker_latent, speaker_mask, latent_out
...
def sample_euler_cfg_independent_guidances(
    ...
    # Clean up KV caches and intermediate tensors
    del kv_text_cond, kv_speaker_cond, kv_text_full, kv_speaker_full
    del full_text_mask, full_speaker_mask, text_mask_uncond, speaker_mask_uncond
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
if __name__ == "__main__":
    from model_manager import get_device
    device = get_device()
    ...
    text_input_ids, text_mask = get_text_input_ids_and_mask([text], max_length=None, device=device)
